...
void BaseDatapath::parse_config(std::string& bench,
  ...
  while (!config_file.eof()) {
    ...
    if (!type.compare("flatten")) {
      ...
      user_params.scratchpad_read_ports = read;
      user_params.scratchpad_write_ports = write;
       user_params.scratchpad_ports = read+write;
    } 
     else if (!type.compare("scratchpad_policy")) {
       sscanf(rest_line.c_str(), "%[^,]\n", user_params.policy);
    }
    else {
