...
ScratchpadDatapath::ScratchpadDatapath(std::string bench,
  ...
  std::cout << "-------------------------------" << std::endl;
  scratchpad = new Scratchpad(user_params.scratchpad_read_ports,
                              user_params.scratchpad_write_ports,
                              user_params.cycle_time,
                              user_params.ready_mode,user_params.policy);
...
void ScratchpadDatapath::scratchpadPartition() {
  ...
  for (auto part_it = user_params.partition.begin();
    ...
    if (!scratchpad_partition_executed) {
      ...
      std::cout<<"Setting the ScratchPad"<<std::endl;
      std::cout<<" array_label, base_addr, part_type, p_factor, size, wordsize"<<std::endl;
      std::cout<< array_label<<","<<base_addr<<","<< part_type<<","<<p_factor<<","<<size<<","<<wordsize<<std::endl;
    } else {
        ...
        try {
          if (scratchpad->canServicePartition(
            ...
            if (isLoad){
              ...
              // std::cout<<"the number of loads are"<<array_name_index;}
            else
